...
class MetricsTable():
    def __init__(self, csv_path: str, table_types: Optional[List[str]] = None):
        ...
        self.columns = ['variant_type', 'variant_size', 'tp', 'fn', 'fp', 'recall', 'precision', 'f1_score']
        # Remove columns that are in columns_to_drop
        self.columns = [column for column in self.columns if column not in self.columns_to_drop]

    def get_df(self):
        return self._df.drop(columns=self.columns_to_drop, errors='ignore')
    ...
    def get_snv_indel_sv(self):
        ...
        return self.get_df()[snv_mask | indel_mask | sv_mask][self.columns]
    ...
    def get_sv_subtypes(self):
        ...
        return self.get_df()[snv_mask | indel_mask | inv_mask | del_mask | ins_mask | dup_mask | tra_mask][self.columns]
    ...
    def get_warnings(self):
        ...
        if 'fp' in df.columns:
            ...
            warning_dict['no_predicted'] = sorted(list(no_predicted))
        ...
        if 'precision' in df.columns:
            ...
            warning_dict['low_precision'] = sorted(list(low_precision - no_truth - no_predicted))
        # Find variant types with recall < RECALL_THRESHOLD
        low_recall = set(df[df['recall'] < PRECISION_THRESHOLD]['variant_type'].tolist())
        warning_dict['low_recall'] = sorted(list(low_recall - no_truth - no_predicted))
        return warning_dict
